...
const apiKey = "Q5m8IktRlxYWyBzraQ7lfl_Yvjp5PWnG0nBmwxY7pZU";
    ...
    searchResults.insertAdjacentHTML(
      ...
      `<div>
        <a href="${unsplashLink}" target="_blank">
          <div class="result-item" style="background-image: url(${url}); background-repeat: no-repeat; background-size: cover;"></div>
        </a>
        <p class="photographer-name">
          <a href="${photographerPage}" target="_blank" style="color: #ffffff;">Photo by ${photographer}</a>
        </p>
      </div>`
    );  
  });
  totalResults = json.total;
  resultStats.textContent = `Найдено ${totalResults} изображений`;
};